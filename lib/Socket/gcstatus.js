...
class GiftedStatus {
    ...
    async sendGroupStatus(groupJid, content, options = {}) {
        ...
        const normalizedGroupJid = WABinary_1.jidNormalizedUser(groupJid);
        ...
        try {
            ...
            this.config.logger?.error?.(`Error getting group metadata: ${error}`);
        }
        ...
        try {
            ...
            this.config.logger?.error?.(`Error generating status message: ${error}`);
            throw error;
        }
        ...
        try {
            ...
            this.config.logger?.error?.(`Error sending group mention: ${error}`);
        }
    ...
    async handleGroupStory(content, jid, quoted) {
        return await this.sendGroupStatus(jid, content.groupStatusMessage || content, {});
    }
}
